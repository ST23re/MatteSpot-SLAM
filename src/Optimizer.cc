...
#include "SparseOptimizer.h" // !IVC-lab@lee
...
namespace ORB_SLAM3
{
	...
	void Optimizer::GlobalBundleAdjustemnt(Map* pMap, int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust, const bool bUseMS)
	{
		vector<KeyFrame*> vpKFs = pMap->GetAllKeyFrames();
		vector<MapPoint*> vpMP = pMap->GetAllMapPoints();
		bUseMS ? MatteSpotBundleAdjustment(vpKFs, vpMP, nIterations, pbStopFlag, nLoopKF, bRobust)
		...
		int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
	{
		...
		// !IVC-lab@lee: check edge chi
		double totalChi = 0;
		for (size_t i = 0, iend = vpEdgesStereo.size(); i < iend; i++)
		{
			g2o::EdgeStereoSE3ProjectXYZ* e = vpEdgesStereo[i];
			double chi = sqrt(e->chi2());
			totalChi += chi;

			MapPoint* pMP = vpMapPointEdgeStereo[i];
			int ID = pMP->GetDescriptor().at<float>(0);

			cout << "!! [Stereo] edge chi: ID{" << ID << "} " << chi << endl;
		}
		cout << "!! [Stereo] Average residual: " << totalChi / vpEdgesStereo.size() << " pixels." << endl;
	...
	void Optimizer::MatteSpotBundleAdjustment(const vector<KeyFrame*>& vpKFs, const vector<MapPoint*>& vpMP,
		int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
	{
		vector<bool> vbNotIncludedMP;
		vbNotIncludedMP.resize(vpMP.size());
		...
		// Circle radius -> K samples
		const double r = 5e-3;		// uint: m
		const int Ksamples = 64;
		std::vector<Eigen::Vector3d> vk(Ksamples);
		for (int k = 0; k < Ksamples; k++)
		{
			double th = 2.0 * M_PI * k / Ksamples;
			vk[k] = Eigen::Vector3d(r * std::cos(th), r * std::sin(th), 0.0);
		}

		vector<EdgeStereoSE3ProjectDot*> vpEdgesStereo; // # 
		vpEdgesStereo.reserve(nExpectedSize);			// #
		vector<KeyFrame*> vpEdgeKFStereo;
		vpEdgeKFStereo.reserve(nExpectedSize);
		vector<MapPoint*> vpMapPointEdgeStereo;
		...
		vector<EdgeDotBoundary*> vpEdgesDotBoundary;			// #
		vpEdgesDotBoundary.reserve(nExpectedSize * Ksamples);	// # !
		vector<KeyFrame*> vpEdgeKFDotBoundary;
		vpEdgeKFDotBoundary.reserve(nExpectedSize * Ksamples);
		vector<MapPoint*> vpMapPointEdgeDotBoundary;
		vpMapPointEdgeDotBoundary.reserve(nExpectedSize * Ksamples);

		vector<EdgeDotNormal*> vpEdgesDotNormal;	// #
		vpEdgesDotNormal.reserve(nExpectedSize);	// #
		vector<KeyFrame*> vpEdgeKFDotNormal;
		vpEdgeKFDotNormal.reserve(nExpectedSize);
		vector<MapPoint*> vpMapPointEdgeDotNormal;
		vpMapPointEdgeDotNormal.reserve(nExpectedSize);

		vector<EdgeDotDepth*> vpEdgesDotDepth;		// #
		vpEdgesDotDepth.reserve(nExpectedSize);		// #
		vector<KeyFrame*> vpEdgeKFDotDepth;
		vpEdgeKFDotDepth.reserve(nExpectedSize);
		vector<MapPoint*> vpMapPointEdgeDotDepth;
		vpMapPointEdgeDotDepth.reserve(nExpectedSize);
		...
		for (size_t i = 0; i < vpMP.size(); i++)
		{
			...
			const auto n = Eigen::Vector3d(desc.at<float>(0, 1), desc.at<float>(0, 2), desc.at<float>(0, 3));

			auto* v = new VertexDot();
			DotEstimate est;
			est.c = pMP->GetWorldPos().cast<double>();
			est.q = quatFromTwoVectors(Eigen::Vector3d(0, 0, 1), n);
			v->setEstimate(est);
			...
			for (map<KeyFrame*, tuple<int, int>>::const_iterator mit = observations.begin(); mit != observations.end(); mit++)
			{
				...
				if (leftIndex != -1) //Stereo observation
				{	/*
					// depth factor 2D
					{
						const cv::KeyPoint& kpUn = pKF->mvKeysUn[leftIndex];
						...
						EdgeStereoSE3ProjectDot* e = new EdgeStereoSE3ProjectDot();

						e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(id)));
						e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(pKF->mnId)));
						e->setMeasurement(obs);

						double sigma_z = kpUn.response * 1e-3; // uint: m
						double sigma_t = 0.1e-3;			   // uint: m
						// sigma_pixel ~= sigma_xyz * fx / Z.
						double sigma = sqrt(sigma_z * sigma_z + sigma_t * sigma_t) * pKF->fx / pKF->mvDepth[leftIndex];
						Eigen::Matrix3d Info = Eigen::Matrix3d::Identity() / (sigma * sigma);
						e->setInformation(Info);
					...
					// boundary factor
					{
						//cv::Mat test = cv::Mat::zeros(pKF->color_KF.size(), CV_8UC3); // visualization
						// reform u_k, n_k :
						const cv::Mat& desc_local = pKF->mDescriptors.row(leftIndex);
						//const cv::KeyPoint& kpUn = pKFi->mvKeysUn[leftIndex];

						auto uk = make_shared<vector<Eigen::Vector2d>>();
						auto nk = make_shared<vector<Eigen::Vector2d>>();

						for (int k = 0; k < 64; ++k) {
							int base = 4 + 4 * k;
							float Px = desc_local.at<float>(0, (base + 0));
							float Py = desc_local.at<float>(0, (base + 1));
							float Gx = desc_local.at<float>(0, (base + 2));
							float Gy = desc_local.at<float>(0, (base + 3));
							uk->push_back(Eigen::Vector2d(Px, Py));
							nk->push_back(Eigen::Vector2d(Gx, Gy));

							//cv::Point2f pt0 = (cv::Point2f(Px, Py) - kpUn.pt) * 10 + kpUn.pt;
							//cv::Point2f pt1(
							//	(pt0.x - Gx * 20.0),
							//	(pt0.y - Gy * 20.0)
							//);
							//cv::circle(test, pt0, 1, cv::Scalar(255.0, 0, 0), -1); // @observation
							//cv::arrowedLine(test, pt0, pt1, cv::Scalar(0, 0, 255), 1, cv::LINE_AA);
						}
						...
						for (int k = 0; k < Ksamples; k++)
						{
							...
							CameraIntrinsics K;
							K.fx = static_cast<double>(pKF->fx);
							K.fy = static_cast<double>(pKF->fy);
							K.cx = static_cast<double>(pKF->cx);
							K.cy = static_cast<double>(pKF->cy);
							auto* e = new EdgeDotBoundary(&K);
							e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(id)));
							e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(pKF->mnId)));
							e->setMeasurement(meas);
							double sigma = 1.0;
							e->setInformation(Eigen::Matrix<double, 1, 1>::Identity() / (sigma * sigma) / Ksamples); // 稳健核+权重在残差里
							auto* huber = new g2o::RobustKernelHuber; huber->setDelta(INFINITY);
							e->setRobustKernel(huber);

							//auto Tcw = static_cast<g2o::VertexSE3Expmap*>(optimizer.vertex(pKFi->mnId))->estimate();
							//auto Dot = static_cast<VertexDot*>(optimizer.vertex(id));
							//const Eigen::Matrix3d Rcw = Tcw.rotation().toRotationMatrix();
							//const Eigen::Vector3d t = Tcw.translation();
							//const Eigen::Matrix3d Rdi = Dot->R();
							//const Eigen::Vector3d o = Rcw * Dot->c() + t;
							//const Eigen::Vector3d p = Dot->c() + Rdi * vk[k];
							//const Eigen::Vector3d x = Rcw * p + t;
							//Eigen::Vector2d u, u_o; Eigen::Matrix<double, 2, 3> Jproj;
							//projectWithJacobian(x, &K, u, Jproj);
							//projectWithJacobian(o, &K, u_o, Jproj);
							//cv::Point2f u_pro;
							//u_pro.x = u_o.x() - (u_o.x() - u.x()) * 10;
							//u_pro.y = u_o.y() - (u_o.y() - u.y()) * 10;
							//cv::circle(test, u_pro, 1, cv::Scalar(0, 255.0, 0), -1); // @estimation

							optimizer.addEdge(e);
							vpEdgesDotBoundary.push_back(e);
							vpEdgeKFDotBoundary.push_back(pKF);
							vpMapPointEdgeDotBoundary.push_back(pMP);
					...
					// normal factor
					{
						...
						const cv::Mat& desc_local = pKF->mDescriptors.row(leftIndex);
						const float& nx = desc_local.at<float>(0, 1);
						const float& ny = desc_local.at<float>(0, 2);
						const float& nz = desc_local.at<float>(0, 3);

						mn.n_cam = Eigen::Vector3d(nx, ny, nz);	// @measurement
						mn.sqrt_w = 1.0;							// 典型上法向约束权重大些

						auto* e = new EdgeDotNormal();
						e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(id)));
						e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(pKF->mnId)));
						e->setMeasurement(mn);
						double sigma = 0.05;
						e->setInformation(Eigen::Matrix3d::Identity() / (sigma * sigma));
						auto* huber = new g2o::RobustKernelHuber; huber->setDelta(thHuber3D);
						e->setRobustKernel(huber);

						//auto Tcw = static_cast<g2o::VertexSE3Expmap*>(optimizer.vertex(pKFi->mnId))->estimate();
						//auto Dot = static_cast<VertexDot*>(optimizer.vertex(id));
						//Eigen::Vector3d nhat = Tcw.rotation().toRotationMatrix() * (Dot->R() * Eigen::Vector3d(0, 0, 1));
						//cout << "!! meas ID: " << pKFi->mDescriptors.at<float>(leftIndex, 0) << " &&  esti ID: " << desc.at<float>(0, 0)
						//	 << " residual: " << (mn.n_cam - nhat).norm() << " rad." << endl;

						//optimizer.addEdge(e);
						vpEdgesDotNormal.push_back(e);
						vpEdgeKFDotNormal.push_back(pKF);
						vpMapPointEdgeDotNormal.push_back(pMP);
					...
					// depth factor 3D
					{
						...
						const cv::KeyPoint& kpUn = pKF->mvKeysUn[leftIndex];
						const float& Z = pKF->mvDepth[leftIndex];
						const float X = (kpUn.pt.x - pKF->cx) * Z * pKF->invfx;
						const float Y = (kpUn.pt.y - pKF->cy) * Z * pKF->invfy;

						md.x_cam = Eigen::Vector3d(X, Y, Z);  // @measurement
						md.sqrt_w = 1.0;                      // 可用置信度调节

						auto* e = new EdgeDotDepth();
						e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(id)));
						e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(pKF->mnId)));
						e->setMeasurement(md);
						...
						double sigma_z = kpUn.response * 1e-3; // uint: m
						double sigma_t = 0.1e-3;			   // uint: m
						e->setInformation(Eigen::Matrix3d::Identity() / (sigma_z * sigma_z + sigma_t * sigma_t));
						auto* huber = new g2o::RobustKernelHuber; huber->setDelta(thHuber3D); // 95% 符合噪声模型的点被当作 inlier，不削权；
						e->setRobustKernel(huber);

						//auto Tcw = static_cast<g2o::VertexSE3Expmap*>(optimizer.vertex(pKFi->mnId))->estimate();
						//auto Dot = static_cast<VertexDot*>(optimizer.vertex(id));
						//Eigen::Vector3d xhat = Tcw.rotation().toRotationMatrix() * Dot->c() + Tcw.translation();
						//cout << "!! meas ID: " << pKFi->mDescriptors.at<float>(leftIndex, 0) << " &&  esti ID: " << desc.at<float>(0, 0)
						//	<< " residual: " << (md.x_cam - xhat).norm() * 1e3 << " mm." << endl;

						optimizer.addEdge(e);
						vpEdgesDotDepth.push_back(e);
						vpEdgeKFDotDepth.push_back(pKF);
						vpMapPointEdgeDotDepth.push_back(pMP);
		...
		for (size_t i = 0, iend = vpEdgesStereo.size(); i < iend; i++)
		{
			EdgeStereoSE3ProjectDot* e = vpEdgesStereo[i];
			double lambda = e->information().trace() / e->dimension(); // 1 / σ^2
			double chi = sqrt(e->chi2() / lambda);
			totalChi += chi;
		...
		if (vpEdgesStereo.size())
			cout << "!! [Stereo] Average residual: " << totalChi / vpEdgesStereo.size() << " pixels." << endl;
		double totalChi3E = 0;
		for (size_t i = 0, iend = vpEdgesDotBoundary.size(); i < iend; i++)
		{
			EdgeDotBoundary* e = vpEdgesDotBoundary[i];
			MapPoint* pMP = vpMapPointEdgeDotBoundary[i];

			double lambda = e->information().trace() / e->dimension(); // 1 / σ^2
			totalChi3E += sqrt(e->chi2() / lambda);
		}
		if (vpEdgesDotBoundary.size())
			cout << "!! [Boundary] Average residual: " << totalChi3E / vpEdgesDotBoundary.size() << " pixels." << endl;

		double totalChi3N = 0;
		for (size_t i = 0, iend = vpEdgesDotNormal.size(); i < iend; i++)
		{
			EdgeDotNormal* e = vpEdgesDotNormal[i]; e->computeError(); // !
			MapPoint* pMP = vpMapPointEdgeDotNormal[i];

			double lambda = e->information().trace() / e->dimension(); // 1 / σ^2
			totalChi3N += sqrt(e->chi2() / lambda);
		}
		if (vpEdgesDotNormal.size())
			cout << "!! [Normal] Average residual: " << totalChi3N / vpEdgesDotNormal.size() << " rad." << endl;

		double totalChi3D = 0;
		for (size_t i = 0, iend = vpEdgesDotDepth.size(); i < iend; i++)
		{
			EdgeDotDepth* e = vpEdgesDotDepth[i];
			MapPoint* pMP = vpMapPointEdgeDotDepth[i];

			double lambda = e->information().trace() / e->dimension(); // 1 / σ^2
			totalChi3D += sqrt(e->chi2() / lambda);
		}
		if (vpEdgesDotDepth.size())
			cout << "!! [Depth] Average residual: " << totalChi3D / vpEdgesDotDepth.size() << " mm." << endl;
		...
		for (size_t i = 0; i < vpKFs.size(); i++)
		{
			...
			else
			{
				...
				{
					int numStereoBadPoints = 0, numStereoOptPoints = 0;
					vector<MapPoint*> vpStereoMPsOpt;

					for (size_t i2 = 0, iend = vpEdgesStereo.size(); i2 < iend; i2++)
					{
						EdgeStereoSE3ProjectDot* e = vpEdgesStereo[i2];
						MapPoint* pMP = vpMapPointEdgeStereo[i2];
						KeyFrame* pKFedge = vpEdgeKFStereo[i2];
		...
		for (size_t i = 0; i < vpMP.size(); i++)
		{
			...
			VertexDot* v = static_cast<VertexDot*>(optimizer.vertex(pMP->mnId + maxKFid + 1));
			Eigen::Vector3f c = v->estimate().c.cast<float>();
			Eigen::Vector3f n = (v->estimate().q.toRotationMatrix() * Eigen::Vector3d(0, 0, 1)).cast<float>();

			if (nLoopKF == pMap->GetOriginKF()->mnId)
			{
				pMP->SetWorldPos(c);
				cv::Mat descHolder;
				pMP->GetDescriptor().copyTo(descHolder);
				descHolder.at<float>(0, 1) = n[0];
				descHolder.at<float>(0, 2) = n[1];
				descHolder.at<float>(0, 3) = n[2];
				pMP->ComputeDistinctiveDescriptors(descHolder);
	...
	void Optimizer::LocalBundleAdjustment(KeyFrame* pKF, bool* pbStopFlag, Map* pMap, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges)
	{
		...
		num_MPs = lLocalMapPoints.size(); // !IVC-lab@lee
		...
		for (size_t i = 0, iend = vpEdgesMono.size(); i < iend; i++)
		{
			...
			cout << "!! [Mono] edge chi:" << sqrt(e->chi2()) << endl;
			if (pMP->isBad())
				continue;
		...
		double totalChi = 0; // !IVC-lab@lee
		for (size_t i = 0, iend = vpEdgesStereo.size(); i < iend; i++)
		{
			g2o::EdgeStereoSE3ProjectXYZ* e = vpEdgesStereo[i];
			MapPoint* pMP = vpMapPointEdgeStereo[i];

			totalChi += sqrt(e->chi2());
			if (pMP->isBad())
				continue;
	...
	void Optimizer::MatteSpotLocalBundleAdjustment(KeyFrame* pKF, bool* pbStopFlag, Map* pMap, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges)
	{
		// Local KeyFrames: First Breath Search from Current Keyframe
		list<KeyFrame*> lLocalKeyFrames;
				...
				if (!pKFi->isBad() && pKFi->GetMap() == pCurrentMap)
				{
					...
					if (leftIndex != -1)
					{
						// depth factor 2D
						{
							...
							double sigma_t = 0.1e-3;			   // uint: m
							// sigma_pixel ~= sigma_xyz * fx / Z.
							double sigma = sqrt(sigma_z * sigma_z + sigma_t * sigma_t) * pKFi->fx / pKFi->mvDepth[leftIndex];
							Eigen::Matrix3d Info = Eigen::Matrix3d::Identity() / (sigma * sigma);
							e->setInformation(Info);
							g2o::RobustKernelHuber* rk = new g2o::RobustKernelHuber;
							e->setRobustKernel(rk);
						...
						// boundary factor
						{
							//cv::Mat test = cv::Mat::zeros(pKF->color_KF.size(), CV_8UC3); // visualization
							// reform u_k, n_k :
							const cv::Mat& desc_local = pKFi->mDescriptors.row(leftIndex);
							//const cv::KeyPoint& kpUn = pKFi->mvKeysUn[leftIndex];
							...
							for (int k = 0; k < Ksamples; k++)
							{
								...
								CameraIntrinsics K;
								K.fx = static_cast<double>(pKFi->fx);
								K.fy = static_cast<double>(pKFi->fy);
								K.cx = static_cast<double>(pKFi->cx);
								K.cy = static_cast<double>(pKFi->cy);
								auto* e = new EdgeDotBoundary(&K);
								e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(id)));
								e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(pKFi->mnId)));
								e->setMeasurement(meas);
								double sigma = 1.0;
								e->setInformation(Eigen::Matrix<double, 1, 1>::Identity() / (sigma * sigma)); // 稳健核+权重在残差里
								auto* huber = new g2o::RobustKernelHuber; huber->setDelta(INFINITY);
								e->setRobustKernel(huber);
								...
								optimizer.addEdge(e);
								vpEdgesDotBoundary.push_back(e);
								vpEdgeKFDotBoundary.push_back(pKFi);
								vpMapPointEdgeDotBoundary.push_back(pMP);
						...
						// normal factor
						{
							...
							double sigma = 0.05;
							e->setInformation(Eigen::Matrix3d::Identity() / (sigma * sigma));
							auto* huber = new g2o::RobustKernelHuber; huber->setDelta(thHuberStereo);
							e->setRobustKernel(huber);
							...
							optimizer.addEdge(e);
							vpEdgesDotNormal.push_back(e);
							vpEdgeKFDotNormal.push_back(pKFi);
							vpMapPointEdgeDotNormal.push_back(pMP);
						...
						/*// depth factor 3D
						{
							...
							const cv::KeyPoint& kpUn = pKFi->mvKeysUn[leftIndex];
							const float& Z = pKFi->mvDepth[leftIndex];
							const float X = (kpUn.pt.x - pKFi->cx) * Z * pKFi->invfx;
							const float Y = (kpUn.pt.y - pKFi->cy) * Z * pKFi->invfy;
							...
							double sigma_t = 0.1e-3;			   // uint: m
							e->setInformation(Eigen::Matrix3d::Identity() / (sigma_z * sigma_z + sigma_t * sigma_t));
							auto* huber = new g2o::RobustKernelHuber; huber->setDelta(thHuberStereo); // 95% 符合噪声模型的点被当作 inlier，不削权；
							e->setRobustKernel(huber);
							...
							optimizer.addEdge(e);
							vpEdgesDotDepth.push_back(e);
							vpEdgeKFDotDepth.push_back(pKFi);
							vpMapPointEdgeDotDepth.push_back(pMP);
		...
		optimizer.computeActiveErrors();                     // 先让每条边重新算误差
		double chi2_init = optimizer.chi2();                 // 总 chi2
		const int ne = static_cast<int>(optimizer.edges().size());
		std::cout << "\nInitial total chi2 = " << chi2_init
			<< ", num edges = " << ne
			<< std::endl;
		...
		vector<pair<KeyFrame*, MapPoint*> > vToErase;
		vToErase.reserve(vpEdgesStereo.size() + vpEdgesDotBoundary.size() + vpEdgesDotNormal.size() /*+ vpEdgesDotDepth.size()*/);
		...
		for (size_t i = 0, iend = vpEdgesStereo.size(); i < iend; i++)
		{
			...
			double lambda = e->information().trace() / e->dimension(); // 1 / σ^2
			totalChi += sqrt(e->chi2() / lambda);
			if (pMP->isBad())
				continue;

			if (e->chi2() > INFINITY || !e->isDepthPositive())
			{
				KeyFrame* pKFi = vpEdgeKFStereo[i];
				vToErase.push_back(make_pair(pKFi, pMP));
			}
		...
		for (size_t i = 0, iend = vpEdgesDotBoundary.size(); i < iend; i++)
		{
			...
			if (e->chi2() > INFINITY)
			{
				KeyFrame* pKFi = vpEdgeKFDotBoundary[i];
				vToErase.push_back(make_pair(pKFi, pMP));
			}
		}
		if (vpEdgesDotBoundary.size())
		...
		for (size_t i = 0, iend = vpEdgesDotNormal.size(); i < iend; i++)
		{
			EdgeDotNormal* e = vpEdgesDotNormal[i];
			MapPoint* pMP = vpMapPointEdgeDotNormal[i];
			...
			if (e->chi2() > INFINITY)
			{
				KeyFrame* pKFi = vpEdgeKFDotNormal[i];
				vToErase.push_back(make_pair(pKFi, pMP));
			}
		}
		if (vpEdgesDotNormal.size())
			cout << "!! [Normal] Average residual: " << totalChi3N / vpEdgesDotNormal.size() * 180 / M_PI << " degrees." << endl;
		...
		for (size_t i = 0, iend = vpEdgesDotDepth.size(); i < iend; i++)
		{
			...
			if (e->chi2() > INFINITY)
			{
				KeyFrame* pKFi = vpEdgeKFDotDepth[i];
				vToErase.push_back(make_pair(pKFi, pMP));
			}
		}
		if (vpEdgesDotDepth.size())
			cout << "!! [Depth] Average residual: " << totalChi3D / vpEdgesDotDepth.size() * 1e3 << " mm." << endl;
