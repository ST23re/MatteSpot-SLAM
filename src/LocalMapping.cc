...
namespace ORB_SLAM3
{
		...
		mIdxInit(0), mScale(1.0), mInitSect(0), mbNotBA1(true), mbNotBA2(true), mIdxIteration(0), infoInertial(Eigen::MatrixXd::Zero(9, 9))
	{
		...
		mbUseMS = pSys->mbUseMS; // !IVC-lab@lee
			...
			if (CheckNewKeyFrames() && !mbBadImu)
			{
				...
				if (!CheckNewKeyFrames() && !stopRequested())
				{
					if (mpAtlas->KeyFramesInMap() > 2)
					{
						...
						{
							if (!mbUseMS) Optimizer::LocalBundleAdjustment(mpCurrentKeyFrame, &mbAbortBA, mpCurrentKeyFrame->GetMap(), num_FixedKF_BA, num_OptKF_BA, num_MPs_BA, num_edges_BA);
							else Optimizer::MatteSpotLocalBundleAdjustment(mpCurrentKeyFrame, &mbAbortBA, mpCurrentKeyFrame->GetMap(), num_FixedKF_BA, num_OptKF_BA, num_MPs_BA, num_edges_BA);
							b_doneLBA = true;

							cc::enable();
							{
								cc::color _(cc::magenta);
								cout << "@LocalBA keyframe_id: " << mpCurrentKeyFrame->mnId
									<< ", num_FixedKF: " << num_FixedKF_BA
									<< ", num_OptKF: " << num_OptKF_BA
									<< ", num_MPs: " << num_MPs_BA
									<< ", num_edges: " << num_edges_BA << endl;
							}
						}
				...
				if (!mbUseMS) mpLoopCloser->InsertKeyFrame(mpCurrentKeyFrame);
				if (b_doneLBA && !mpCurrentKeyFrame->depth_KF.empty() && !mpCurrentKeyFrame->color_KF.empty())
					mpSystem->RenderFrame(mpCurrentKeyFrame); // !IVC-lab@lee
	...
	void LocalMapping::ProcessNewKeyFrame()
	{
		...
		if (!mbUseMS) mpCurrentKeyFrame->ComputeBoW();
			...
			if (pMP)
			{
				if (!pMP->isBad())
				{
					if (!pMP->IsInKeyFrame(mpCurrentKeyFrame))
					{
						...
						mbUseMS ? void() : pMP->ComputeDistinctiveDescriptors();
					}
	...
	void LocalMapping::CreateNewMapPoints()
	{
		...
		if (!mbUseMS) {
			...
			/*
				//  当前关键帧有地图点未包含的新点
				cout << "NewKeyFrame -- hasValidPose: " << mpCurrentKeyFrame->hasValidMSTransform << endl;
				if (!mpCurrentKeyFrame->hasValidMSTransform) return;
				...
				const int gN = globalMapPoints.size();
				//// Debugging
				//Eigen::Matrix4f Twc = mpCurrentKeyFrame->GetPoseInverse().matrix();
				//cout << "mpCurrentKeyFrame->pose: frameId=" << mpCurrentKeyFrame->mnFrameId << endl << Twc << endl;
				//for (vector<MapPoint*>::iterator iter = globalMapPoints.begin(); iter != globalMapPoints.end(); iter++) {
				//	MapPoint* pMP = *iter;
				//	//Eigen::Vector3f x3D = pMP->GetWorldPos();
				//	//cv::Mat desc = pMP->GetDescriptor();
				//	//cout << "< MapPoint > x: " << x3D.x() << ", y: " <<  x3D.y() << ", z: " << x3D.z()
				//	//    << ", nx: " << desc.at<float>(1) << ", ny: " << desc.at<float>(2) << ", nz: " << desc.at<float>(3)
				//	//    << ", id: " << desc.at<float>(0)
				//	//    << endl
				//}
				...
				for (int i = 0; i < mpCurrentKeyFrame->N; i++)
				{
					...
					mpCurrentKeyFrame->UnprojectRGBD(i, x3D, n3D);

					float& ID = mpCurrentKeyFrame->mDescriptors.at<float>(i, 0);
					...
					if (ID <= 0) {
						...
						for (int j = 0; j < gN; j++) {
							...
							double d = (pMP->GetWorldPos() - x3D).norm();
						...
						if (novel) {
							...
							pMP->ComputeDistinctiveDescriptors(desc);
							pMP->UpdateNormalAndDepth();

							mpAtlas->AddMapPoint(pMP);
							mlpRecentAddedMapPoints.push_back(pMP);

							cout << "LM: New MapPoint created!" << endl;
						}
					}
				}
			*/
