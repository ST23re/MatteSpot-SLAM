...
namespace ORB_SLAM3
{
	...
	void Tracking::newParameterLoader(Settings* settings) {
		...
		//Matte Spot parameters
		mbUseMS = mpSystem->mbUseMS;
	}
	...
	Sophus::SE3f Tracking::GrabImageRGBD(FrameData* mpFrameData, string filename) // !IVC-lab@lee
	{
		mImGray = mpFrameData->t_u8.clone();
		cv::Mat imDepth = mpFrameData->d_f32.clone();
		cv::Mat imNorm = mpFrameData->n_f32.clone();
		cv::Mat imConf = mpFrameData->c_f32.clone();
		double timestamp = mpFrameData->frameTimestamp;

		if ((fabs(mDepthMapFactor - 1.0f) > 1e-5) || imDepth.type() != CV_32F)
			imDepth.convertTo(imDepth, CV_32F, mDepthMapFactor);

		if (mbUseMS) mCurrentFrame = Frame(mImGray, imDepth, imNorm, imConf, timestamp, mK, mDistCoef, mbf, mThDepth, mpCamera);
		else mCurrentFrame = Frame(mImGray, imDepth, timestamp, mpORBextractorLeft, mpORBVocabulary, mK, mDistCoef, mbf, mThDepth, mpCamera);
	...
	void Tracking::Track()
	{
		...
		else
		{
			...
			{
				// Update motion model ---- 基于与上一帧的相对位姿变化指定运动速度
...
#endif
				bool bNeedKF = NeedNewKeyFrame() || true;
	...
	void Tracking::TrackMS() // !IVC-lab@lee
	{
		if (bStepByStep)
		{
			std::cout << "Tracking: Waiting to the next step" << std::endl;
		...
		else
		{
			...
			if (!mbOnlyTracking)
			{
				...
				if (!bOK)
					cout << "Fail to update local map!" << endl;
			}
			else
			{
				if (bOK)
			...
			if (bOK || mState == RECENTLY_LOST)
			{
				...
				std::chrono::steady_clock::time_point time_StartNewKF = std::chrono::steady_clock::now();
#endif
				bool bNeedKF = NeedNewKeyFrameMS();
			...
			//if (mState == LOST)
			//{
			//    if (pCurrentMap->KeyFramesInMap() <= 3)
			//    {
			//        mpSystem->ResetActiveMap();
			//        return;
			//    }
	...
	void Tracking::MSInitialization() {
		if (mCurrentFrame.N > 5)
		{
			mCurrentFrame.SetPose(Sophus::SE3f());
			mCurrentFrame.hasValidMSTransform = true;
			// Dispatch ID
			MSmatcher& matcher = MSmatcher::getInstance();
			unique_lock<mutex> lock(matcher.mutex_MS);
			for (int i = 0; i < mCurrentFrame.N; i++)
			{
				float& ID = mCurrentFrame.mDescriptors.row(i).at<float>(0);
				ID = matcher.getNewID();

				bool excluded = std::find(matcher.excluded.begin(), matcher.excluded.end(), ID) != matcher.excluded.end();
				if (excluded) {
					auto MS = mCurrentFrame.mergedCenters[i];
					std::ostringstream oss;
					oss << " " << mCurrentFrame.mTimeStamp << " " << ID
						<< setprecision(9) << " " << MS.p.x << " " << MS.p.y << " " << MS.p.z
						<< " " << MS.n.x << " " << MS.n.y << " " << MS.n.z;
					matcher.AppendExcludedMeasurement(oss.str());
				}
			}

			// Create KeyFrame
			KeyFrame* pKFini = new KeyFrame(mCurrentFrame, mpAtlas->GetCurrentMap(), mpKeyFrameDB);
			FrameData* mpFD = mpSystem->mpFrameData;
			if (!mpFD->d_f32.empty() && !mpFD->t_u8.empty())
			{
				cv::Mat depth_mt;
				/*mCurrentFrame.ZeroDepthInEllipses(mpFD->d_f32.clone(), depth_mt);*/
				mCurrentFrame.CorrDepthInEllipses(mpFD->d_f32.clone(), depth_mt);
				pKFini->depth_KF = depth_mt;
				//pKF->depth_KF = mpFD->d_f32.clone();
				pKFini->color_KF = mpFD->t_u8.clone();
				pKFini->normal_KF = mpFD->n_f32.clone();
				pKFini->confid_KF = mpFD->c_f32.clone();
				mCurrentFrame.GetMaskMS(mpFD->t_u8.size(), pKFini->mask_KF);
				pKFini->frameIndex = mpFD->frameIndex;
			}
			...
			for (int i = 0; i < mCurrentFrame.N; i++)
			{
				...
				if (z > 0)
				{
					...
					//pNewMP->ComputeDistinctiveDescriptors(); this step find the best desriptor for fast matching 
					pNewMP->ComputeDistinctiveDescriptors(mCurrentFrame.mDescriptors.row(i)); // Temp
					pNewMP->UpdateNormalAndDepth(); // TODO: normal vertex & scale level
			...
			cc::enable();
			{ cc::color _(cc::red); cout << "Insufficient matte spots to initialize!" << endl; }
		}
	}
	...
	void Tracking::UpdateLastFrame()
	{
		...
		Sophus::SE3f Tlr = mlRelativeFramePoses.back();      // |<-- Tlr -->|
		mLastFrame.SetPose(Tlr * pRef->GetPose()); // ......, refKey, ..., last
	...
	bool Tracking::TrackLocalMap()
	{
		...
		else // !IVC-lab@lee
		{
			...
			if (!mCurrentFrame.hasValidMSTransform) return false; // this should not happen

			MSmatcher& matcher = MSmatcher::getInstance();

			Map* pCurrentMap = mpAtlas->GetCurrentMap();
			vector<MapPoint*> globalMapPoints = pCurrentMap->GetAllMapPoints();

			matcher.SearchByProjection(mCurrentFrame, globalMapPoints);
		}
		...
		if (mbUseMS) return true; // !IVC-lab@lee
	...
	bool Tracking::NeedNewKeyFrameMS() // !IVC-lab@lee
	{
		if (mbOnlyTracking)
			return false;
		...
		if (!mpSystem->mbOnline)
			return true;

		const int nKFs = mpAtlas->KeyFramesInMap();

		// Do not insert keyframes if not enough frames have passed from last keyframe
		if (mCurrentFrame.mnId < mnLastKeyFrameId + mMaxFrames / 2 && nKFs > mMaxFrames / 2)
		{
			return false;
		}
		...
		Eigen::Vector3f Owi = mCurrentFrame.GetCameraCenter();
		Eigen::Vector3f Owr = mpReferenceKF->GetCameraCenter();
		float dOw = (Owi - Owr).norm(); // m

		// Check MS ID overlap ratio {notice: ID can be 0, -1, N+}
		cv::Mat IDi = mCurrentFrame.mDescriptors.col(0);
		cv::Mat IDr = mpReferenceKF->mDescriptors.col(0);
		MSmatcher& matcher = MSmatcher::getInstance();
		float nmatchesRatio = matcher.GetOverlapRatio(IDi, IDr);

		// Condition 1a: More than "MaxFrames" have passed from last keyframe insertion
		const bool c1a = mCurrentFrame.mnId >= mnLastKeyFrameId + mMaxFrames;
		// Condition 1b: Local Mapping is idle
		const bool c1b = bLocalMappingIdle; //mpLocalMapper->KeyframesInQueue() < 2);
		// Condition 1c: Have a certain deviation
		const bool c1c = dOw >= 0.1f;

		// Condition 2: ID overlap ratio is less than or equal to 60%
		const bool c2 = nmatchesRatio <= 0.6f;

		cout << "dOw=" << dOw << "; nmatchesRatio=" << nmatchesRatio << endl;
		cout << "NewKF Conditions: c1a=" << c1a << "; c1b=" << c1b << "; c1c=" << c1c << "; c2=" << c2 << endl;
	...
	void Tracking::CreateNewKeyFrame()
	{
		...
		if (!mpFD->d_f32.empty() && !mpFD->t_u8.empty())
		{
			...
			/*mCurrentFrame.ZeroDepthInEllipses(mpFD->d_f32.clone(), depth_mt);*/
			mCurrentFrame.CorrDepthInEllipses(mpFD->d_f32.clone(), depth_mt);
			pKF->depth_KF = depth_mt;
			//pKF->depth_KF = mpFD->d_f32.clone();
			pKF->color_KF = mpFD->t_u8.clone();
			pKF->normal_KF = mpFD->n_f32.clone();
			pKF->confid_KF = mpFD->c_f32.clone();
			mCurrentFrame.GetMaskMS(mpFD->t_u8.size(), pKF->mask_KF);
			pKF->frameIndex = mpFD->frameIndex;
		}
		...
		if (mSensor != System::MONOCULAR && mSensor != System::IMU_MONOCULAR) // TODO check if incluide imu_stereo
		{
			...
			if (!mbUseMS) {
				...
				// cout << "create new MPs" << endl;
				//cout << "NewKeyFrame -- hasValidPose: " << pKF->hasValidMSTransform << endl;
				if (!pKF->hasValidMSTransform) return; // this should not happen

				// Debugging
				/*Eigen::Matrix4f Twc = pKF->GetPoseInverse().matrix();
				cout << "pKF->pose: frameId=" << pKF->mnFrameId << endl << Twc << endl;*/
				...
				Map* pCurrentMap = mpAtlas->GetCurrentMap();
				vector<MapPoint*> globalMapPoints = pCurrentMap->GetAllMapPoints();
				const int gN = globalMapPoints.size(); // need to be set first

				for (int i = 0; i < pKF->N; i++)
				{
					Eigen::Vector3f x3D, n3D;
					pKF->UnprojectRGBD(i, x3D, n3D);

					float& ID = pKF->mDescriptors.at<float>(i, 0); // attention, this is ref

					if (ID > 0) continue;
					// Debugging
					/*cout << "> KeyPoint < x: " << x3D.x() << ", y: " << x3D.y() << ", z: " << x3D.z()
						<< ", nx: " << n3D.x() << ", ny: " << n3D.y() << ", nz: " << n3D.z()
						<< endl;*/

					bool novel = true;
					for (int j = 0; j < gN; j++)
					{
						MapPoint* pMP = globalMapPoints[j];
						double dp = (pMP->GetWorldPos() - x3D).norm();
						//cout << "checkpoint: " << dp << endl;
						if (dp < 0.02) // max center distance between two MS on plane
						{
							novel = false;
							break;
						}
					...
					if (novel)
					{
						...
						pNewMP->AddObservation(pKF, i);
						pKF->AddMapPoint(pNewMP, i);
						cv::Mat desc = (cv::Mat_<float>(1, 4) << ID, n3D.x(), n3D.y(), n3D.z());
						pNewMP->ComputeDistinctiveDescriptors(desc);
						pNewMP->UpdateNormalAndDepth();

						mpAtlas->AddMapPoint(pNewMP);
						cout << "New MapPoint created, ID: " << ID << endl;

						mCurrentFrame.mDescriptors.at<float>(i, 0) = ID;
						mCurrentFrame.mvpMapPoints[i] = pNewMP;
					}
				}
			}
	...
	bool Tracking::MSRelocalization()
	{
		Verbose::PrintMess("Starting relocalization", Verbose::VERBOSITY_NORMAL);

		if (mCurrentFrame.N < 4) return false;

		MSmatcher& matcher = MSmatcher::getInstance();

		int nmatches = 0;
		nmatches = matcher.SetMSCorrespondences(mCurrentFrame, mvpLocalMapPoints);
		bool byLocal = nmatches >= 4;
		if (!byLocal)
		{
			Map* pCurrentMap = mpAtlas->GetCurrentMap();
			vector<MapPoint*> globalMapPoints = pCurrentMap->GetAllMapPoints();

			nmatches = matcher.SetMSCorrespondences(mCurrentFrame, globalMapPoints);
		}
		if (nmatches < 4) return false;

		int nGood = Optimizer::PoseOptimization(&mCurrentFrame);
		cc::enable(); { cc::color _(cc::magenta); cout << "@Tracking::Pose-only BA: nGood is " << nGood << endl; }
		...
		{
			mnLastRelocFrameId = mCurrentFrame.mnId;
			cout << "Relocalized using " << (byLocal ? "Local" : "Global") << " points!!" << endl;
			/*cout << "frame: " << mCurrentFrame.mnId << endl;
			cout << mCurrentFrame.GetPose().matrix() << endl;
			for (int i = 0; i < mCurrentFrame.N; i++)
			{
				cv::Mat& mDesc = mCurrentFrame.mDescriptors.row(i);
				if (mDesc.at<float>(0, 0) <= 0) continue;
				for (int j = 0; j <= 3; j++)
					cout << mDesc.at<float>(0, j) << " ";
