...
#define FRAME_GRID_COLS 64
	...
	struct MatteSpot {
		...
		int count;
		float confidence;
		cv::Point2f c;							// [Disorted] Projection center
		cv::Point3d p;							// [Camera space]
		cv::Point3d n;							// [Camera space]
		cv::RotatedRect ellipse;				// [Disorted] only for visualization
		// To eliminate measurement errors introduced by spot.
		cv::RotatedRect ellipse_outer;			// [Disorted]
		// For boundary opt & photometric computation.
		cv::RotatedRect ellipseUn;				// [Undisorted]
		std::vector<cv::Point2f> subpixelP;		// uk
		std::vector<cv::Vec2f> subpixelG;		// nk
		MatteSpot(cv::RotatedRect e, cv::RotatedRect e_un) :
			ellipse(e), ellipse_outer(e), ellipseUn(e_un), ID(-1), count(1), confidence(.0f), c(.0f, .0f), p(0.0, 0.0, 0.0), n(0.0, 0.0, 0.0),
			subpixelP(std::vector<cv::Point2f>()), subpixelG(std::vector<cv::Vec2f>())
		{};
	};
	//typedef shared_ptr<MatteSpot> MatteSpotptr;
	...
	public:
		...
		Frame(const cv::Mat& imGray, const cv::Mat& imDepth, const cv::Mat& imNorm, const cv::Mat& imConf, const double& timeStamp, cv::Mat& K, cv::Mat& distCoef, const float& bf, const float& thDepth, GeometricCamera* pCamera, Frame* pPrevF = static_cast<Frame*>(NULL), const IMU::Calib& ImuCalib = IMU::Calib());
		...
		// Extract Matte Spots on the image.
		void ExtractMatteSpots(const cv::Mat& imGray, const cv::Mat& imDepth, const cv::Mat& imNorm, const cv::Mat& imConf);
		void SetMSkeysDescripters();
		// Evaluation
		void GetMaskMS(const cv::Size size, cv::Mat& dst);
		void PreserveIntensityInEllipses(const cv::Mat& gray_src, cv::Mat& gray_dst);
		void ZeroDepthInEllipses(const cv::Mat& depth_src, cv::Mat& depth_dst);
		void CorrDepthInEllipses(const cv::Mat& depth_src, cv::Mat& depth_dst);
	...
	public:
		...
		// !IVC-lab@lee
		vector<MatteSpot> mergedCenters;
		bool hasValidMSTransform;
